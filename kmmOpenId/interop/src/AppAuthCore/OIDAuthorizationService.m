...
+ (BOOL)URL:(NSURL *)URL matchesRedirectionURL:(NSURL *)redirectionURL {
    ...
    return [standardizedURL.scheme caseInsensitiveCompare:standardizedRedirectURL.scheme] ==
           NSOrderedSame
 ...
 */
- (void)didFinishWithResponse:(nullable OIDAuthorizationResponse
...
/*! @brief Invokes the pending callback and performs cleanup.
 ...
 @param error The error, if any, to return to the callback.
 */
- (void)didFinishWithResponse:(nullable OIDEndSessionResponse
            ...
            [session dataTaskWithURL:discoveryURL
                   completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                       ...
                       if (urlResponse.statusCode != 200) {
                           NSError *
                           URLResponseError = [OIDErrorUtilities HTTPErrorWithHTTPResponse:urlResponse
                                                                                      data:data];
                           NSString * errorDescription =
...
+ (void)performRegistrationRequest:(OIDRegistrationRequest *)request
    ...
    if (!URLRequest) {
        // A problem occurred deserializing the response/JSON.
        NSError *
        returnedError = [OIDErrorUtilities errorWithCode:OIDErrorCodeJSONSerializationError
                                         underlyingError:nil
    ...
    [[session dataTaskWithRequest:URLRequest
                completionHandler:^(NSData *_Nullable data,
                    ...
                    if (error) {
                        ...
                        NSString * errorDescription =
                                [NSString stringWithFormat:@"Connection error making registration request to '%@': %@.",
                                                           ...
                                                           error.localizedDescription];
                        NSError *
                        returnedError = [OIDErrorUtilities errorWithCode:OIDErrorCodeNetworkError
                                                         underlyingError:error
                                                             description:errorDescription];
                        dispatch_async(dispatch_get_main_queue(), ^{
                            completion(nil, returnedError);
                    ...
                    if (HTTPURLResponse.statusCode != 201 && HTTPURLResponse.statusCode != 200) {
                        // A server error occurred.
                        NSError *
                        serverError = [OIDErrorUtilities HTTPErrorWithHTTPResponse:HTTPURLResponse
                                                                              data:data];
                        ...
                        NSString * errorDescription =
                                [NSString stringWithFormat:@"Non-200/201 HTTP response (%d) making registration request "
                                                           ...
                                                           URLRequest.URL];
                        NSError *
                        returnedError = [OIDErrorUtilities errorWithCode:OIDErrorCodeServerError
                                                         underlyingError:serverError
                                                             description:errorDescription];
                        dispatch_async(dispatch_get_main_queue(), ^{
                            completion(nil, returnedError);
                    ...
                    if (jsonDeserializationError) {
                        ...
                        NSString * errorDescription =
                                [NSString stringWithFormat:@"JSON error parsing registration response: %@",
                                                           jsonDeserializationError.localizedDescription];
                        NSError *
                        returnedError = [OIDErrorUtilities errorWithCode:OIDErrorCodeJSONDeserializationError
                                                         underlyingError:jsonDeserializationError
                                                             description:errorDescription];
                        dispatch_async(dispatch_get_main_queue(), ^{
                            completion(nil, returnedError);
